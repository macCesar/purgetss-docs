...
In Alloy, unlike the [official icon fonts](icon-fonts-libraries), custom fonts generate `./purgetss/styles/fonts.tss`. That file is folded into the generated `app/styles/app.tss` when you run `purgetss` (or compile your app). Classic projects skip this step and use the font's PostScript name directly through Titanium's `fontFamily` property.
:::
...
```javascript title="app/lib/purgetss.fonts.js (Alloy) or Resources/lib/purgetss.fonts.js (Classic)"
const icons = {
  // map-icons/map-icons.css
  'mapIcon': {
    'abseiling': '\ue800',
