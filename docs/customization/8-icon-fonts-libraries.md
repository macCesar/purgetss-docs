...
Run [`icon-library`](../commands#icon-library-command) to copy the `.ttf` files into `./app/assets/fonts/`. That is the only step. Once the fonts are in place, the icon classes from the table above work.
