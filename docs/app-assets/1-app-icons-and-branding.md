...
On Android, add these items to the app's existing theme:

```xml title="Existing Android theme"
<!-- Android 12+ system splash -->
<item name="android:windowSplashScreenBackground">#0B1326</item>

<!-- Native window before Titanium draws the first Window -->
<item name="android:windowBackground">#0B1326</item>
```

The theme must already be referenced by the `<application>` element in `tiapp.xml`:

```xml title="./tiapp.xml"
<application android:theme="@style/YourExistingTheme">
```

For Alloy projects, Android theme resources normally live under `app/platform/android/res/values/`. Classic projects use `platform/android/res/values/`. If a theme already exists, merge the two items into its existing `<style>`; do not add another `<application>` element or replace the theme wholesale.

Run `purgetss brand --notes` after generating the assets to print these snippets with the project's current `brand.colors.background` value. PurgeTSS only prints the guidance and never edits either platform configuration automatically.
