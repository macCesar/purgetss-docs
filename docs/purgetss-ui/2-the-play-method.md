...
Callbacks receive `type`, `bubbles`, `cancelBubble`, `action`, `state`, `id`, `targetId`, `index`, `total`, and `getTarget()`. For an array, the callback runs once per view. Use [`sequence`](additional-methods#the-sequence-method) when each view must finish before the next begins.

See [Using `purgetss.ui` in Titanium Classic](titanium-classic#states-and-child-animations) for child animation objects and lifecycle guidance.
