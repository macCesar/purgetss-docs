...
### Arguments
  ...
  - `Project's Name` enclosed in single or double quotes. *This argument is required*

### Options

  - `-f, --force` to overwrite an existing project
  - `-t, --tailwind` to install **Tailwind CSS** in your project
  - `-v, --vendor [fa,mi,ms,f7]` to copy the selected fonts into your project, including the CommonJS module into `./app/lib/` folder. View the **[`copy-fonts` command](#copy-fonts-command)**  for available fonts!
...
It generates shades and tints for a given color and outputs the corresponding color palette in `config.js`.
...
More than 66% of all `tailwind.tss` classes are related to color properties, so having a tool like `shades` to extend them with new colors is a great addition to **PurgeTSS**!
...
Use the `--log` option to output to the console instead of saving them to the `config.js` file.
...
To generate a random color value use the `--random` option, here, we are using the `--log` option to log it to the console
...
Use the `--json` option to log a Titanium's `config.json` compatible structure to the console
...
**You won't need to manually download or copy them into the appropriate folder in your project.**
...
### Options and Flags
  ...
  - `-s, --styles` to copy the corresponding `tss` files into the `./purgetss/styles/` folder for you to review
...
After copying the desired fonts, you can use them in `Buttons` and `Labels`. For instance, to use **FontAwesome**, just set the font family to `fa` (for Solid icons) and the desired icon, such as `fa-home`, for example.
...
:::tip PRO TIP
If you want to use a shorter name for any of the `fontFamily` classes, you can simply rename the font file to the desired name.
